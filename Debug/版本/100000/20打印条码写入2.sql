if (exists (select * from sys.objects where name = 'proc_PDABarCodeSign_Insert21'))
    drop proc proc_PDABarCodeSign_Insert21
go
create proc proc_PDABarCodeSign_Insert21
(
...
declare  @FShortBarCode varchar(128),--短条码
         ...
         @FRemark4 varchar(255),--辅助标识,
          ...
          @FAllHouDu varchar(502), --厚度合并
          @FAllChangDu varchar(502), --长度合并
          @FAllChenShu varchar(502), --层数合并
          @FAllKuanDu varchar(502), --宽度平均值合并
          @FAllGuiGe varchar(502), --规格
...
set @F_FFF_INTEGER = dbo.getString(@mainStr,'|',11)  --原木长默认0
set @F_FFF_INTEGER1 = dbo.getString(@mainStr,'|',12) --原木直径默认0
set @F_FFF_INTEGER2 = dbo.getString(@mainStr,'|',13) --板长默认0
set @F_FFF_INTEGER3 = dbo.getString(@mainStr,'|',14) --板宽默认0
set @F_FFF_INTEGER4 = dbo.getString(@mainStr,'|',15) --板厚默认0
set @F_FFF_Volume = dbo.getString(@mainStr,'|',16) --总体积
...
set @F_Plies = dbo.getString(@mainStr,'|',22)-- 总层数 
set @FBatcnNoIndex = dbo.getString(@mainStr,'|',23) --批号附加值
set @FAllHouDu = dbo.getString(@mainStr,'|',24) --厚度合并
set @FAllChangDu = dbo.getString(@mainStr,'|',25) --长度合并
set @FAllChenShu = dbo.getString(@mainStr,'|',26) --层数合并
set @FAllKuanDu = dbo.getString(@mainStr,'|',27) --宽度平均值合并
set @FAllGuiGe = dbo.getString(@mainStr,'|',28) --规格
